function ind2bad = scroll_artificial_channels2(raw,lims,badch)
    ...
    if sum(1 == badch) % If first channel is marked bad
        p=plot(ax,raw(1,:),'Color','r');ylim(lims);xlim([1 size(raw,2)]);
        badchanlist(1) = 1;
    else
        p=plot(ax,raw(1,:),'Color','k');ylim(lims);xlim([1 size(raw,2)])
    end
    ...
    function SliderChangeFunction(hObject,eventdata)
        ...
        if sum(ind2plot == badch) % Mark as bad if in list.
            p=plot(ax,raw(ind2plot,:),'Color','r');ylim(lims);xlim([1 size(raw,2)]);
            badchanlist(ind2plot) = 1;
        else
            p=plot(ax,raw(ind2plot,:),'Color','k');ylim(lims);xlim([1 size(raw,2)]);
        end
        
        p2 = plot(zoomed,raw(ind2plot,1:round(size(raw,2)/50)),'Color','k');ylim(zoomed,get(zoomed,'YLim'));
    end
